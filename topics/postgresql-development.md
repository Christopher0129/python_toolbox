# PostgreSQL 开发专题

这个专题关注 PostgreSQL 在 Python 项目里的建模、查询、事务和批处理写入实践。

## 常见主题

- 参数化 SQL
- 事务边界
- 索引设计
- 批量导入导出
- ORM 与原生 SQL 的取舍

## 关联阅读

- [psycopg](../third_party/psycopg.md)
- [psycopg_pool](../third_party/psycopg_pool.md)
- [SQLModel](../third_party/sqlmodel.md)
- [SQLAlchemy 2.x 基础](../database/sqlalchemy.md)
- [SQL 优化专题](./sql-optimization.md)
- [PostgreSQL 锁与并发专题](./postgresql-locking.md)
- [PostgreSQL Vacuum 与膨胀专题](./postgresql-vacuum-bloat.md)
- [PostgreSQL WAL 与 Checkpoint 专题](./postgresql-wal-checkpoint.md)
- [PostgreSQL 备份与恢复专题](./postgresql-backup-restore.md)
- [PostgreSQL 复制与故障切换专题](./postgresql-replication-failover.md)
- [PostgreSQL 复制延迟专题](./postgresql-replication-lag.md)
- [PostgreSQL 读写分离专题](./postgresql-read-write-splitting.md)
- [PostgreSQL 分区维护专题](./postgresql-partition-maintenance.md)
- [PostgreSQL 逻辑解码专题](./postgresql-logical-decoding.md)
- [PostgreSQL 连接耗尽专题](./postgresql-connection-exhaustion.md)
- [PostgreSQL 死锁排障专题](./postgresql-deadlock-troubleshooting.md)
- [PostgreSQL 长事务专题](./postgresql-long-transactions.md)
- [PostgreSQL 批量导入专题](./postgresql-bulk-ingest.md)
- [PostgreSQL 幂等写入专题](./postgresql-idempotent-writes.md)
- [PostgreSQL 任务队列专题](./postgresql-job-queues.md)
- [PostgreSQL Advisory Lock 专题](./postgresql-advisory-locks.md)
- [PostgreSQL 语句超时专题](./postgresql-statement-timeouts.md)
- [PostgreSQL 统计与观测专题](./postgresql-stats-observability.md)
- [PostgreSQL 租户隔离专题](./postgresql-tenant-isolation.md)
- [事务与隔离级别专题](./transaction-isolation.md)

返回 [索引](../README.md)
